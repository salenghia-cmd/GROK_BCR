...
dependencies {
    // Các dependency gốc của BCR (giữ nguyên như file cũ của bạn)
    ...
    // ──────────────────────────────────────────────
    // Thêm mới cho tính năng realtime bridge (pha 1 & pha 2)
    // ──────────────────────────────────────────────

    // OkHttp + WebSocket (cần cho kết nối tới server từ dt1)
    implementation("com.squareup.okhttp3:okhttp:4.12.0")

    // Logging interceptor (rất hữu ích khi debug WebSocket)
    implementation("com.squareup.okhttp3:logging-interceptor:4.12.0")

    // JSON parsing đơn giản, nhẹ (dùng cho handshake hello, session info)
    implementation("org.json:json:20240303")   // phiên bản mới nhất tính đến 2025

    // Coroutines (khuyên dùng ngay từ đầu để RealtimeBridgeService sạch sẽ hơn)
    implementation("org.jetbrains.kotlinx:kotlinx-coroutines-android:1.8.1")  // hoặc 1.8.1 nếu muốn ổn định hơn
