...
class SettingsFragment : PreferenceBaseFragment(),
    ...
    // Realtime bridge
    private lateinit var prefEnableRealtimeBridge: SwitchPreferenceCompat
    private lateinit var prefRealtimeOutUrl: EditTextPreference

    // Optional prefs: có thể chưa tồn tại trong root_preferences.xml
    private var prefDeviceId: EditTextPreference? = null
    private var prefPlaybackMode: Preference? = null
    private var prefInjectMode: Preference? = null
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        // Realtime bridge
        prefEnableRealtimeBridge = findPreference(Preferences.PREF_ENABLE_REALTIME_BRIDGE)!!
        prefRealtimeOutUrl = findPreference(Preferences.PREF_REALTIME_OUT_URL)!!

        // Optional: chỉ bind nếu XML có thật
        prefDeviceId = findPreference(Preferences.PREF_DEVICE_ID)
        prefPlaybackMode = findPreference(Preferences.PREF_PLAYBACK_MODE)
        prefInjectMode = findPreference(Preferences.PREF_INJECT_MODE)

        refreshRealtimeBridgeSummary()
        refreshDebugPrefs()
    ...
    private fun refreshLauncherIcon() {
        prefShowLauncherIcon.isChecked = prefs.showLauncherIcon
    }
    ...
    private fun refreshRealtimeBridgeSummary() {
        val enabled = prefEnableRealtimeBridge.isChecked
        val url = prefRealtimeOutUrl.text ?: "Chưa cấu hình"

        prefRealtimeOutUrl.summary = if (enabled) {
            "Đang bật - URL: $url"
        } else {
            "Tắt"
        }

        prefDeviceId?.summary = prefs.deviceId ?: "android"
        prefPlaybackMode?.summary = prefs.playbackMode
        prefInjectMode?.summary = prefs.injectMode
    }
    ...
    override fun onPreferenceChange(preference: Preference, newValue: Any?): Boolean {
        ...
        when (preference) {
            prefCallRecording -> {
                if (newValue == false || Permissions.haveRequired(context)) {
                    return true
                } else {
        ...
        when (preference) {
            ...
            prefOutputDir -> {
                OutputDirectoryBottomSheetFragment().show(
                    ...
                    OutputDirectoryBottomSheetFragment.TAG,
                )
                return true
            }
            ...
            prefOutputFormat -> {
                OutputFormatBottomSheetFragment().show(
                    childFragmentManager,
                    OutputFormatBottomSheetFragment.TAG,
                )
                return true
            }
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: String?) {
        when (key) {
            ...
            // Update the switch state if it was toggled outside of the preference
            // (eg. from the quick settings toggle).
            ...
            Preferences.PREF_OUTPUT_DIR,
            Preferences.PREF_OUTPUT_RETENTION -> {
                refreshOutputDir()
            }
            ...
            Preferences.PREF_ENABLE_REALTIME_BRIDGE,
            Preferences.PREF_REALTIME_OUT_URL,
            Preferences.PREF_DEVICE_ID,
            Preferences.PREF_PLAYBACK_MODE,
            Preferences.PREF_INJECT_MODE -> {
                refreshRealtimeBridgeSummary()
            }

            else -> {
                if (Preferences.isFormatKey(key)) {
                    refreshOutputFormat()
                }
            }
        }
