...
class RealtimeBridgeService : Service() {
    ...
    companion object : RealtimePcmSink {
        ...
        fun sanitizeBridgeSampleRate(sampleRate: Int): Int {
            return when {
                sampleRate <= 0 -> 8000
        ...
        fun sanitizeBridgeChannels(channels: Int): Int {
            return 1
        }
    }
    ...
    private fun handleCallStarted(intent: Intent) {
        ...
        val requestedSampleRate = intent.getIntExtra(EXTRA_SAMPLE_RATE, 8000)
        val requestedChannels = intent.getIntExtra(EXTRA_CHANNELS, 1)
        val sampleRate = sanitizeBridgeSampleRate(requestedSampleRate)
        val channels = sanitizeBridgeChannels(requestedChannels)

        if (requestedSampleRate != sampleRate || requestedChannels != channels) {
            Log.w(
                TAG,
                "Bridge format sanitized request=${requestedSampleRate}/${requestedChannels} -> runtime=${sampleRate}/${channels}"
            )
        }
        ...
        Log.i(
            TAG,
            "Bridge started for session=$sessionId callId=$callId sampleRate=${info.sampleRate} channels=${info.channels} playbackMode=${info.playbackMode} injectMode=${info.injectMode}"
        )
        if (info.injectMode.equals("call_tx", ignoreCase = true) || info.injectMode.equals("vendor_incall", ignoreCase = true)) {
            Log.i(TAG, "call_tx will try direct AudioAttributes call-redirection first, then AudioPolicy injector; không fallback local playback")
        }
    }
    ...
    private class SessionRuntime(
        ...
        private val playbackThread = PlaybackThread(
            ...
            routeMode = info.playbackMode,
        )

        private val uplinkInjector = UplinkInjector(
            context = appContext,
        ...
        private val bridgeRecorder = BridgeMixedWavRecorder(
            context = appContext,
        ...
        @Volatile
        private var injectorStarted = false
        ...
        fun connect() {
            ...
            if (!injectorStarted) {
                try {
                    uplinkInjector.start()
                    injectorStarted = true
                } catch (t: Throwable) {
                    Log.e(TAG, "Failed to start uplink injector for session=${info.sessionId}", t)
                }
            }
            ...
            socket = client.newWebSocket(request, object : WebSocketListener() {
                ...
                override fun onMessage(webSocket: WebSocket, bytes: ByteString) {
                    ...
                    bridgeRecorder.appendInbound(pcm)
                    uplinkInjector.handleInboundPcm(pcm)
                }
        ...
        fun sendOutboundPcm(pcm: ByteArray, sampleRate: Int, channels: Int) {
            ...
            val normalizedPcm = normalizePcm16Le(
                pcm = pcm,
            ...
            if (sampleRate != info.sampleRate || channels != info.channels) {
                Log.w(
                    TAG,
                    "PCM format mismatch runtime=${info.sampleRate}/${info.channels} actual=$sampleRate/$channels -> normalized=${info.sampleRate}/${info.channels}"
                )
            }

            bridgeRecorder.appendOutbound(normalizedPcm)
            current.send(ByteString.of(*normalizedPcm))
        }

        private fun normalizePcm16Le(
            pcm: ByteArray,
            inputSampleRate: Int,
            inputChannels: Int,
            ...
            val safeInputChannels = max(1, inputChannels)
            val safeOutputChannels = max(1, outputChannels)

            if (pcm.size < 2) return ByteArray(0)

            val frameCount = pcm.size / 2 / safeInputChannels
            ...
            for (frame in 0 until frameCount) {
                ...
                mono[frame] = (sum / safeInputChannels).coerceIn(-1f, 1f)
            }

            val resampled = resampleMonoPcm(
                samples = mono,
                inputSampleRate = inputSampleRate,
                outputSampleRate = outputSampleRate,
            )

            if (resampled.isEmpty()) return ByteArray(0)

            val out = ByteArray(resampled.size * safeOutputChannels * 2)
            var outIndex = 0

            for (sample in resampled) {
                val clamped = sample.coerceIn(-1f, 1f)
                val pcm16 = if (clamped < 0f) {
                    (clamped * 32768f).toInt()
                } else {
                    (clamped * 32767f).toInt()
                }.coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt())

                repeat(safeOutputChannels) {
        ...
        private fun resampleMonoPcm(
            ...
            val inRate = if (inputSampleRate > 0) inputSampleRate else outputSampleRate
            val outRate = if (outputSampleRate > 0) outputSampleRate else inputSampleRate

            if (samples.isEmpty() || inRate <= 0 || outRate <= 0 || inRate == outRate) {
                return samples
            }

            val ratio = inRate.toDouble() / outRate.toDouble()
            val outLength = max(1, kotlin.math.round(samples.size / ratio).toInt())
            val out = FloatArray(outLength)

            for (i in 0 until outLength) {
                val position = i * ratio
                val left = min(samples.size - 1, position.toInt())
                val right = min(samples.size - 1, left + 1)
                val frac = position - left
                out[i] = (samples[left] * (1.0 - frac) + samples[right] * frac).toFloat()
            }
