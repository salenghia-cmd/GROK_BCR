...
class RealtimeBridgeService : Service() {
    ...
    private fun handleCallStarted(intent: Intent) {
        ...
        Log.i(
            TAG,
            "Bridge started for session=$sessionId callId=$callId playbackMode=${info.playbackMode} injectMode=${info.injectMode}"
        )
        if (info.injectMode.equals("vendor_incall", ignoreCase = true)) {
            Log.w(TAG, "vendor_incall hiện chưa inject thật trong repo này; nếu chưa có nhánh vendor riêng thì web -> máy kia vẫn chỉ là best-effort local playback")
        }
    }
