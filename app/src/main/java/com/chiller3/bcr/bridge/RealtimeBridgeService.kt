...
class RealtimeBridgeService : Service() {
    ...
    private fun handleCallStarted(intent: Intent) {
        ...
        if (info.injectMode.equals("vendor_incall", ignoreCase = true)) {
            Log.i(TAG, "vendor_incall will try hidden PSTN call injection first, then fallback local playback if ROM / permission does not allow it")
        }
    }
