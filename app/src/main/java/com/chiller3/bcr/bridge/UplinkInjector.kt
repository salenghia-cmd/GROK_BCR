...
class UplinkInjector(
    ...
    private val injectMode: String = "bluetooth_sco",
) {
    companion object {
    ...
    private class PlaybackRouteInjector(
        private val playbackThread: PlaybackThread,
        private val label: String,
    ) : InboundAudioInjector {
        override fun start() {
            Log.i(TAG, "$label started")
        }
        ...
        override fun stop() {
            Log.i(TAG, "$label stopped")
        }

        companion object {
            private const val TAG = "PlaybackRouteInjector"
        }
    }

    /**
     * Best-effort inject thật vào uplink PSTN bằng hidden call-redirection AudioTrack.
     * Nếu máy / ROM / quyền không hỗ trợ thì fallback về local playback theo route hiện tại.
     */
        ...
        private fun tryCreateCallInjectionTrack(): AudioTrack? {
            return try {
                ...
                val setCallRedirectionMode = AudioTrack.Builder::class.java.getDeclaredMethod(
                    "setCallRedirectionMode", Int::class.javaPrimitiveType,
                )
        ...
        private fun downmixToMono16Le(bytes: ByteArray, inputChannels: Int): ByteArray {
            ...
            repeat(frameCount) {
                ...
                repeat(inputChannels) {
                    ...
                    val hi = bytes[inIndex + 1].toInt() and 0xff
    ...
    private fun normalizeInjectMode(raw: String): String {
        return when (raw.trim().lowercase()) {
            "bluetooth_sco", "bt_sco", "sco", "hfp", "hfp_sco", "auto", "call", "follow_call", "" -> "bluetooth_sco"
            "vendor_incall" -> "vendor_incall"
            "speaker_loopback", "speaker", "local" -> "speaker_loopback"
            "noop" -> "noop"
            else -> "bluetooth_sco"
        }
    }
    ...
    private val impl: InboundAudioInjector by lazy {
        when (normalizeInjectMode(injectMode)) {
            "noop" -> {
                Log.w(TAG, "injectMode=noop -> sẽ drop audio từ web xuống handset")
                NoopInjector()
            }
            "speaker_loopback" -> PlaybackRouteInjector(playbackThread, "SpeakerLoopbackInjector")
            ...
            else -> PlaybackRouteInjector(playbackThread, "BluetoothScoInjector")
        }
    }
