...
class RecorderInCallService : InCallService(), RecorderThread.OnRecordingCompletedListener {
    ...
    /**
     * Pha 2: sessionId tương ứng với từng call đang được bridge realtime.
     */
     ...
     * telephony framework. As a result, [handleStateChange] stops the recording if the call enters
    ...
    private fun handleStateChange(call: Call, state: Int?) {
        ...
        // Pha 2: đẩy state hiện tại sang bridge service
    ...
    private fun startRecording(call: Call) {
        ...
        // Cho phép mọi phone package để dễ test bridge
        if (callPackage != PHONE_PACKAGE) {
            Log.w(TAG, "Non-default phone package: $callPackage, but allowing it")
        }

        val callId = getCallId(call)
        val sessionId = RealtimeBridgeService.ensureSessionId(callId)
     ...
     * [notificationIdsToRecorders] to keep the foreground service alive until the
     * [RecorderThread] exits and reports its status. The thread may exit and be removed from
     * [callsToRecorders] before this function is called if an error occurs during recording.
     *
    ...
    private fun requestStopRecording(call: Call) {
        ...
        // Pha 2: stop bridge khi call kết thúc / bị huỷ
