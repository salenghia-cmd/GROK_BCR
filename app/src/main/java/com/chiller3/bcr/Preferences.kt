...
class Preferences(initialContext: Context) {
    companion object {
        ...
        private val VALID_PLAYBACK_MODES = setOf("earpiece", "speaker", "speaker_loopback", "follow_call")
        private val VALID_INJECT_MODES = setOf("call_tx", "speaker_loopback", "local", "noop")
    ...
    var playbackMode: String
        ...
        set(value) = prefs.edit {
            ...
            putString(
                PREF_PLAYBACK_MODE,
                if (normalized in VALID_PLAYBACK_MODES) normalized else "earpiece",
            )
        }
    ...
    var injectMode: String
        get() {
            ...
            val normalized = when (raw) {
                null, "", "auto", "follow_call", "call", "vendor_incall" -> "call_tx"
                "speaker_loopback", "local", "noop", "call_tx" -> raw
                else -> "call_tx"
            }
        ...
        set(value) = prefs.edit {
            ...
            putString(
                PREF_INJECT_MODE,
                if (normalized == "vendor_incall") "call_tx" else if (normalized in VALID_INJECT_MODES) normalized else "call_tx",
            )
        }
