...
class Preferences(initialContext: Context) {
    companion object {
        ...
        private val VALID_PLAYBACK_MODES = setOf("bluetooth_sco", "earpiece", "speaker", "speaker_loopback", "follow_call")
        private val VALID_INJECT_MODES = setOf("bluetooth_sco", "vendor_incall", "speaker_loopback", "local", "noop")
    ...
    var playbackMode: String
        get() {
            val raw = prefs.getString(PREF_PLAYBACK_MODE, null)
                ?.trim()
                ?.lowercase()

            val normalized = normalizePlaybackMode(raw)
        ...
        set(value) = prefs.edit {
            putString(PREF_PLAYBACK_MODE, normalizePlaybackMode(value.trim().lowercase()))
        }
    ...
    var injectMode: String
        get() {
            ...
            val normalized = normalizeInjectMode(raw)
        ...
        set(value) = prefs.edit {
            putString(PREF_INJECT_MODE, normalizeInjectMode(value.trim().lowercase()))
        }

    fun prefersBluetoothScoBridge(): Boolean {
        return playbackMode == "bluetooth_sco" || injectMode == "bluetooth_sco"
    }

    private fun normalizePlaybackMode(raw: String?): String {
        return when (raw) {
            null, "", "auto", "call", "bt_sco", "sco", "hfp", "hfp_sco", "bluetooth", "bluetooth_sco" -> "bluetooth_sco"
            "speaker_loopback" -> "speaker_loopback"
            "follow_call" -> "follow_call"
            "speaker" -> "speaker"
            "earpiece", "local" -> "earpiece"
            else -> if (raw in VALID_PLAYBACK_MODES) raw else "bluetooth_sco"
        }
    }

    private fun normalizeInjectMode(raw: String?): String {
        return when (raw) {
            null, "", "auto", "follow_call", "call", "bt_sco", "sco", "hfp", "hfp_sco", "bluetooth", "bluetooth_sco" -> "bluetooth_sco"
            "speaker_loopback", "local", "speaker" -> "speaker_loopback"
            "noop", "vendor_incall" -> raw
            else -> if (raw in VALID_INJECT_MODES) raw else "bluetooth_sco"
        }
    }
    ...
    var outputRetention: Retention?
        get() = getOptionalUint(PREF_OUTPUT_RETENTION, UInt.MAX_VALUE)
            ?.let { Retention.fromRawPreferenceValue(it) }
    ...
    var recordRules: List<RecordRule>?
        get() = prefs.getString(PREF_RECORD_RULES, null)?.let {
