...
class Preferences(initialContext: Context) {
    companion object {
        ...
        private val VALID_INJECT_MODES = setOf("vendor_incall", "speaker_loopback", "local", "noop")
    ...
    var injectMode: String
        get() {
            ...
            val normalized = when (raw) {
                null, "", "auto", "follow_call", "call" -> "vendor_incall"
                "speaker_loopback", "local", "noop", "vendor_incall" -> raw
                else -> "vendor_incall"
            }
        ...
        set(value) = prefs.edit {
            ...
            putString(
                PREF_INJECT_MODE,
                if (normalized in VALID_INJECT_MODES) normalized else "vendor_incall",
            )
        }
