...
private fun connectWebSocket() {
    ...
    Log.d(tag, "Sent PCM chunk: ${pcmBytes.size} bytes via WebSocket")
}
    ...
    private fun encodeLoop(
        ...
        while (!isCancelled || !buffersFlushed) {
            ...
            if (nWritten > 0 && webSocket != null) {
                val pcmBytes = ByteArray(nWritten)
                bufferOut.position(0)
                bufferOut.get(pcmBytes, 0, nWritten)
                bufferOut.position(nWritten)  // khôi phục cho encoder
                sendPcmChunk(pcmBytes)
            }
