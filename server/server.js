...
if (!fs.existsSync(KEY_FILE) || !fs.existsSync(CERT_FILE)) {
  console.error('[fatal] Missing TLS certificate files.');
  console.error(`[fatal] Expected: ${KEY_FILE}`);
  console.error(`[fatal] Expected: ${CERT_FILE}`);
  process.exit(1);
}
...
function handleHttpRequest(req, res, isHttps) {
  ...
  if (!isHttps) {
    ...
    const location = `https://${hostOnly}:${HTTPS_PORT}${req.url || '/'}`;
...
httpServer.listen(HTTP_PORT, HOST, () => {
  ...
  console.log(`It will redirect to HTTPS on port ${HTTPS_PORT}`);
});

httpsServer.listen(HTTPS_PORT, HOST, () => {
  console.log(`HTTPS server listening on https://${HOST}:${HTTPS_PORT}/index.html`);
  console.log(`WSS endpoint: wss://<LAN-IP>:${HTTPS_PORT}/ws?session=<session-id>`);
});